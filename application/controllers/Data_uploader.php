...
class Data_uploader extends CI_Controller
{
    ...
    public function process_jpeg_upload($image_id="CIL_0")
    {
        error_reporting(E_ALL ^ E_WARNING);
        $this->load->helper('url');
        $dbutil = new DB_util();
        ...
        if($num<=$min_upload_id)
        {
            $message = '{"jsonrpc" : "2.0", "error" : {"code": 103, "message": "Invalid image ID: '.$num.'."}, "id" : '.$num.'}';
            //$message = "Invalid Image ID:"+$num;
            error_log("\nUpload error: ID is ".$num, 3, $targetDir."/upload_jpeg.log");
            $this->output->set_status_header('400');
            //header('Content-Type: application/json');
            die($message);
            return;
        ...
        if(!file_exists($uploadLog))
        {
            ...
            //$message = "Invalid Image ID:"+$num;
            error_log("\nUpload error: ID is ".$num, 3, $targetDir."/upload_jpeg.log");
            error_log("\n".$message, 3, $targetDir."/upload_jpeg.log");
            $this->output->set_status_header('400');
            //header('Content-Type: application/json');
            die($message);
            return;
        ...
        error_log("\ntargetDir:".$targetDir, 3, $targetDir."/upload_jpeg.log");
        ...
        if (isset($_FILES['file']['tmp_name']) && is_uploaded_file($_FILES['file']['tmp_name'])) 
            error_log("\nStep 0: Filename:".$fileName, 3, $targetDir."/upload_jpeg.log");
        ...
        error_log("\nStep 0", 3, $topDir."/upload_jpeg.log");
                
        if (strpos($contentType, "multipart") !== false) 
        {
            error_log("\nStep 1:", 3, $topDir."/upload_jpeg.log");

            if (isset($_FILES['file']['tmp_name']) && is_uploaded_file($_FILES['file']['tmp_name'])) 
            {
                error_log("\nStep 1.1 - upload file:".$_FILES['file']['tmp_name'], 3, $topDir."/upload_jpeg.log");
                // Open temp file
                $out = fopen($targetDir . DIRECTORY_SEPARATOR . $fileName, $chunk == 0 ? "wb" : "ab");
                if ($out) 
                {
                    ...
                    {
                         error_log("\n".$e->getMessage(), 3, $targetDir."/upload_jpeg.log");
                    }
                }
                else
                {
        ...
        {
            error_log("\nStep 2", 3, $imagesFolder."/upload_jpeg.log");
            // Open temp file
            $out = fopen($targetDir . DIRECTORY_SEPARATOR . $fileName, $chunk == 0 ? "wb" : "ab");
            if ($out) 
            {
