...
class Cdeep3m_preview extends CI_Controller
{
    ...
    public function new_images_job()
    {
        $this->load->helper('url');
        $base_url = $this->config->item('base_url');
        $dbutil = new DB_util();
        ...
        $crop_id = $dbutil->getNextCropID();
        redirect ($base_url."/cdeep3m_preview/upload_images_job/".$crop_id);

    }
    
    public function upload_images_job($crop_id)
    {
        $this->load->helper('url');
        $base_url = $this->config->item('base_url');
        $dbutil = new DB_util();
        ...
        $data['title'] = 'Home > Upload images';
        $this->load->view('templates/header', $data);
        $this->load->view('cdeep3m/job/images_upload_job_display', $data);
        $this->load->view('templates/footer', $data);
    }
    
    
    public function select_parameters_job($crop_id)
    {
        ...
        $data['title'] = 'Home > Select parameters';
        $this->load->view('templates/header', $data);
        $this->load->view('cdeep3m/job/images_select_parameters', $data);
        $this->load->view('templates/footer', $data);
    }
    
    
    public function submit_preview_job($crop_id)
    {
        $this->load->helper('url');
        $base_url = $this->config->item('base_url');
        $dbutil = new DB_util();
        ...
        */
        ...
        $image_service_auth = $this->config->item('image_service_auth');
        $image_service_prefix = $this->config->item('image_service_prefix');
        $image_service_url = $image_service_prefix."/Cdeep3m_prp_job_service/image_preview_step2/stage/".$crop_id."/".$docker_image_type;
        //echo "<br/>image_service_url:".$image_service_url;
        $response = $cutil->auth_curl_post($image_service_url, $image_service_auth,"");
        }
        //echo "<br/>Response:".$response;
        ...
        $data['email'] = $email;
        $this->load->view('templates/header', $data);
        $this->load->view('cdeep3m/job/submitted_parameters_display', $data);
        $this->load->view('templates/footer', $data);
    }
}
